...
class VehicleRegistrationViewController : UIViewController, UIScrollViewDelegate, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITableViewDelegate, UITableViewDataSource
{
    ...
    override func viewDidLoad() {
        ...
        var vehicleCategory = ""
        var show_VehType = true
        switch (def.integer(forKey: "ProductCode"))
        {
        case 1:
            vehicleCategory = Constant.twowheeler
            break
        case 2:
            vehicleCategory = Constant.privatecar
            break
        case 9:
            show_VehType = false
            vehicleCategory = Constant.passCom
            break
        case 10:
            show_VehType = false
            vehicleCategory = Constant.goodCom
            break
        case 17:
            show_VehType = false
            vehicleCategory = Constant.miscCom
            break
        default:
            break
        }
        if (!show_VehType)
        {
        let usageTypeURL = Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor + vehicleCategory + Constant.VehUsagetype+Constant.companyC
    ...
    {
        APICallered().fetchData(Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor+Constant.hypothet+Constant.companyC+companyName) { response in
        ...
        APICallered().fetchData(Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor+vehicleType+Constant.vehBodyType) { response in
            self.typeArray = response?.object(forKey: "Response") as? NSArray
        }
        
        APICallered().fetchData(Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor+Constant.vehColor) { response in
    ...
    private func applyShadowToViews()
    {
        ...
        common.applyBorderToView(vehUsageType, withColor: Colors.textFldColor, ofSize: 1)
    }
    ...
    private func applyRoundToViews()
    {
        ...
        common.applyRoundedShapeToView(vehUsageType, withRadius: 5)
    ...
    @IBAction func continueTapp(_ sender: UIButton) {
        ...
        vehUsageType.resignFirstResponder()
        def.synchronize()
    ...
    func hideMyHypoView(_ hide: Bool) {
        if (hide)
        {
            hypoShow.constant = 0.1
        }
        else
        {
            hypoShow.constant = 152
        }
    ...
    func hideMyPUCView(_ hide: Bool) {
        if (hide)
        {
            ...
            pucPolShow.constant = 0.1
        }
        else
        {
            pucPosition.constant = 10
            pucPolShow.constant = 220
        }
    ...
    private func validateVehicleNumber()
    {
        ...
        apiCall += Constant._subprod+common.sharedUserDefaults().string(forKey: "SubProductCode")!
        APICallered().fetchData(apiCall) { response in
            ...
            else
            {
                DispatchQueue.main.async {
                    ...
                    alertVu.showInfo("Alert!", subTitle: msg, closeButtonTitle: "Okay", timeout: .none, colorStyle: UInt(self.common.colorValue), colorTextButton: .max, circleIconImage: nil, animationStyle: .noAnimation)
                }
            }
        }
    }
    ...
    func textFieldDidChangeSelection(_ textField: UITextField) {
        ...
        case 176:
                apiArray = nil
                apiArray = NSMutableArray.init()
            apiArray.addObjects(from: Common.financeCityArray as! [Any])
                let selectArr: NSArray = apiArray
                let str = textField.text! as String
                let emptyArr = NSMutableArray.init()
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField)-> Bool
    {
        ...
        default:
                ...
                apiArray = nil
                apiArray = NSMutableArray.init()
                if (Common.financeCityArray != nil){
                    apiArray.addObjects(from: Common.financeCityArray as! [Any])
                    apiTableView.reloadData()
                    apiTableView.updateTableContentInset()
                    apiTable.isHidden = false
