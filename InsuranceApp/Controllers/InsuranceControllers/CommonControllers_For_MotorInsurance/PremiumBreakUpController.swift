...
//        " ₹"
        ...
        let m = Constant.buyAPI+Constant.baseURL+Constant.companylogo+(companyDictionary?.object(forKey: "CompanyCode") as! String)+".png"
 ...
 as! String
        let ccc = CGFloat(idvV.count/60)
        var hhh = 90.0
        if (ccc > 1)
        {
            hhh += 20*ccc
        }
        addonPremHigh.constant = hhh
