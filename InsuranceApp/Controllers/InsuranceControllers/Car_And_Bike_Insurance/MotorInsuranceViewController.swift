...
class MotorInsuranceViewController: UIViewController, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate
{
    ...
    private func loadMyCarArrays()
    {
        ...
        var productName = Constant.twowheeler
        if (sub_prod_id == 2)
        {
            productName = Constant.privatecar
        }
        ...
        // for manufacturer
        APICallered().fetchData(Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor+vehicle_type+Constant.manufacturer) { response in
            Common.manufacturerArray = response?.object(forKey: "Response") as? NSArray
        }

        // for model
        APICallered().fetchData(Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor+Constant.model+Constant.subProd_id+String(sub_prod_id)) { response in
            Common.modelArray = response?.object(forKey: "Response") as? NSArray
        }
        
        // for variant
        APICallered().fetchData(Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor+Constant.variant+Constant.subProd_id+String(sub_prod_id)) { response in
        ...
        APICallered().fetchData(Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor+productName+Constant.probusRenewal) { response in
...
//        var constraintV: NSLayoutConstraint!
        if (tappedValue == 1)
        {
            quotationViewShow.constant = 0.1
            ...
            APICallered().fetchData(apiCall) { response in
                ...
                if (valid)
                {
                    DispatchQueue.main.async { [self] in
                        let detailURL = Constant.buyAPI+Constant.baseURL+Constant.home+Constant.VehRegiDetail+Constant.vehNo+vehNumber+Constant.tokenNum+token
...
//        let quotationDetails = "PIBLMTRPC2021120813525876"
        ...
        if (v)
        {
            APICallered().fetchData(Constant.buyAPI+Constant.baseURL+Constant.forApi+Constant.motor+Constant.quotationDetails+Constant.quot_Num+string) { response in
                DispatchQueue.main.async {

                let k = (response?.object(forKey: "Response") as? NSArray)
                if (k != nil)
                {
                    let msgStr = response?.object(forKey: "Message") as? String
                    if (msgStr != nil)
                    {
                        if (!(msgStr!.elementsEqual("No data found.")))
                        {
                            let anotherk = k!.object(at: 0) as? NSDictionary
                            let m = anotherk?.object(forKey: "Request") as? String
    ...
    @IBAction func getDetailsOfPolicyTapped(_ sender: UIButton) {
        ...
        if (vehicleRegisterTwoField.hasText && privacyPolicyField.hasText && !selectPrvacyLbl.text!.elementsEqual("Select Privacy Policy"))
        {
            ...
            var apiCall = Constant.buyAPI+Constant.baseURL+Constant.home+Constant.validVehCheck+Constant.vehNo+k!
            apiCall = apiCall+Constant._subprod+def.string(forKey: "SubProductCode")!
            ...
            APICallered().fetchData(apiCall) { response in
                ...
                if (valid)
                {
                    DispatchQueue.main.async { [self] in
                        ...
                        var apiurl = Constant.apiAPI+Constant.baseURL+Constant.forApi+Constant.motor+productName+Constant.GetDetProbusRenew+Constant.brokerId+"PIBL"+Constant._device+"3"+Constant._prevInsurerCode+String(self.common.sharedUserDefaults().integer(forKey: "previousInsurerCode"))
                        apiurl += Constant._prevPolicyNumb+self.privacyPolicyField.text!
                        apiurl += Constant._regNumb+vehicleRegisterTwoField.text!
...
*/
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (tappedValue == indexPath.row)
        {
            ...
            if (!k)
            {
                common.sharedUserDefaults().removeObject(forKey: "IsQuotation")
            }
