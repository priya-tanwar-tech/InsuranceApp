...
class MotorGetQuoteViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate, UICollectionViewDelegateFlowLayout, UITextFieldDelegate
{
    ...
    private func setNCB()
    {
        ...
        dateDiff = Calendar.current.dateComponents([.year], from: vehicleDate, to:Date() ).year ?? 0
        if (dateDiff == 2)
        {
            tappedNCB = 1
        }
...
//        PAShow = constraintV
//        constraintV = nil
                NSLayoutConstraint.activate([ ownerTop])
    ...
    func hideMyOwnerShowView(_ hide: Bool) {
        ...
        if (hide)
        {
            constraintV = common.ShowAndHideView(ConstarintIs: ownerShow, isHidden: hide, andSize: 0.1)
        }
        else
        {
            constraintV = common.ShowAndHideView(ConstarintIs: ownerShow, isHidden: hide, andSize: 1)
        }
   ...
   func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
       ...
       case 1405:
               if (tappedNCB == indexPath.row)
               {
                   ...
                   if (dateDiff > 1)
                   {
                       if (indexPath.row == (Common.ncbArray.count-1))
                       {
                           newNCB = Int(Common.ncbArray[tappedNCB].replacingOccurrences(of: "%", with: ""))
                       }
                       else
                       {
                           newNCB = Int(Common.ncbArray[tappedNCB+1].replacingOccurrences(of: "%", with: ""))
                       }
                   }
                   else
                   {
