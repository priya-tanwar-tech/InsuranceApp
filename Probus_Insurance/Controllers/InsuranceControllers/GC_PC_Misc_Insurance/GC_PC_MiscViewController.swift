...
class GC_PC_MiscViewController : UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate
{
    ...
    {        
        ...
        var api_url = common.A_P_I+common.baseURL+common.forApi+common.motor+common.gc_pc_misc_insurance+common.subProd_ID
    ...
    private func loadMyCarArrays()
    {
        ...
        APICallered().fetchData(common.A_P_I+common.baseURL+common.forApi+common.motor+vehicle_type+common.manufacturer+common.companyId+companyId) { response in
            Common.manufacturerArray = response?.object(forKey: "Response") as? NSArray
        }
        // for model
        APICallered().fetchData(common.A_P_I+common.baseURL+common.forApi+common.motor+common.model+common.subProd_ID+sub_prod_id+common.and_companyId+companyId) { response in
            Common.modelArray = response?.object(forKey: "Response") as? NSArray
        }
        // for variant
        APICallered().fetchData(common.A_P_I+common.baseURL+common.forApi+common.motor+common.variant+common.subProd_ID+sub_prod_id+common.and_companyId+companyId) { response in
            ...
            APICallered().fetchData(apiCall) { response in
                ...
                if (valid)
                {
                    DispatchQueue.main.async { [self] in
                        APICallered().fetchData(self.common.buy+self.common.baseURL+self.common.home+self.common.VehilceRegisterationDetail+self.common.vNo+vehNumber+self.common.tokenNumber+token) { response in
