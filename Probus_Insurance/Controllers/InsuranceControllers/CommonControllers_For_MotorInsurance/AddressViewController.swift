...
class AddressViewController : UIViewController, UIScrollViewDelegate, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITableViewDataSource, UITableViewDelegate
{
    ...
    override func viewDidLoad() {
        ...
        APICallered().fetchData(common.A_P_I+common.baseURL+common.forApi+common.motor+common.stateBy+common.company+k!) { response in
    ...
    func hideMyView(_ hide: Bool) {
        ...
        if (hide)
        {
            constraintV = common.ShowAndHideView(ConstarintIs: permanentShow, isHidden: hide, andSize: 0.00000001)
        }
        else
        {
            constraintV = common.ShowAndHideView(ConstarintIs: permanentShow, isHidden: hide, andSize: 0.75)
        }
        permanent.isHidden = hide
        permanentShow.isActive = false
        permanentShow = nil
        permanentShow = constraintV
        constraintV = nil
        NSLayoutConstraint.activate([permanentShow])
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField)-> Bool
    {
        ...
        case 175:
                ...
                if ((CommAddDet.object(forKey: "StateCode")) != nil)
                {
                    ...
                    var cmpny : String = common.A_P_I+common.baseURL+common.forApi+common.motor+common.cityBy+common.company+k+common.stateCode+ANOTHERSTR!
                    cmpny += common.subproduct+def.string(forKey: "SubProductCode")!
                    
                    APICallered().fetchData(cmpny) { response in
                        Common.cityArray = response?.object(forKey: "Response") as? NSArray

                        if (Common.cityArray != nil)
                        {
                            DispatchQueue.main.async {
                                self.apiArray = NSMutableArray.init()
                                self.apiArray.addObjects(from: Common.cityArray as! [Any])
        ...
        case 181:
                ...
                if ((VehAddDet.object(forKey: "StateCode")) != nil)
                {
                    ...
                    cmpny += common.subproduct+def.string(forKey: "SubProductCode")!
                    APICallered().fetchData(cmpny) { response in
                        self.cityArr = response?.object(forKey: "Response") as? NSArray
                        if (self.cityArr != nil)
                        {
                            DispatchQueue.main.async {
                                self.apiArray = NSMutableArray.init()
                                self.apiArray.addObjects(from: self.cityArr as! [Any])
                                self.apiTableView.reloadData()
                                self.apiTableView.updateTableContentInset()
                                self.apiTable.isHidden = false
    ...
    func textFieldDidChangeSelection(_ textField: UITextField) {
        if (textField.tag == 180 || textField.tag == 181 || textField.tag == 174 || textField.tag == 175)
        {
            ...
            case .cit:
                ...
                break
            case .citP:
                apiArray.addObjects(from: self.cityArr as! [Any])
                break
            default:
                break
            }
        ...
        case 176:
                ...
                {
                    let k = common.buy+common.baseURL+common.home+common.pincode_check+textField.text!
