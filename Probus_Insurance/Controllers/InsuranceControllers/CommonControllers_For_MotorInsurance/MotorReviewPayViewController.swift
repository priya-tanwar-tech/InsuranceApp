...
class MotorReviewPayViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource
{
    ...
    private let payArr = ["Credit/Debit Card", "Net Banking", "EMI"]
...
//        holderOcc.text = ((clientDetails.object(forKey:"Occupation") as! NSDictionary).object(forKey: "Name") as! String)
    ...
    {
        let urlString = common.A_P_I+common.baseURL+common.forApi+common.motor
        let companySubProd = common.company+cName!+common.subproduct+def.string(forKey: "SubProductCode")!
    ...
    func hideMyAppointeeView(_ hide: Bool) {
        ...
        if (hide)
        {
            ...
            constraintV = common.ShowAndHideView(ConstarintIs: appointeeHigh, isHidden: hide, andSize: 0.000000001)
        }
        else
        {
            appointeeTop.constant = 10
            constraintV = common.ShowAndHideView(ConstarintIs: appointeeHigh, isHidden: hide, andSize: 0.18)
        }
        appointeeHigh.isActive = false
        appointeeHigh = nil
        appointeeHigh = constraintV
        constraintV = nil
        NSLayoutConstraint.activate([appointeeHigh, appointeeTop])
    ...
    func hideMyPolicyView(_ hide: Bool) {
        ...
        if (hide)
        {
            ...
            constraintV = common.ShowAndHideView(ConstarintIs: policyHigh, isHidden: hide, andSize: 0.000000001)
        }
        else
        {
            policyTop.constant = 10
            constraintV = common.ShowAndHideView(ConstarintIs: policyHigh, isHidden: hide, andSize: 0.21)
        }
        policyHigh.isActive = false
        policyHigh = nil
        policyHigh = constraintV
        constraintV = nil
        NSLayoutConstraint.activate([policyHigh, policyTop])
    ...
    func hideMyHypoView(_ hide: Bool) {
        ...
        {
            hypoTop.constant = 0
            constraintV = common.ShowAndHideView(ConstarintIs: hypoHigh, isHidden: hide, andSize: 0.000000001)
        }
        else
        {
            hypoTop.constant = 10
            constraintV = common.ShowAndHideView(ConstarintIs: hypoHigh, isHidden: hide, andSize: 0.21)
        }
        hypoHigh.isActive = false
        hypoHigh = nil
        hypoHigh = constraintV
        constraintV = nil
        NSLayoutConstraint.activate([hypoHigh, hypoTop])
    ...
    private func inspectionViewSize(_ hide : Bool)
    {
        ...
        if (!hide)
        {
            ...
            inspectionHeight = 0.2
            if (agncArr != nil)
            {
                inspectionHeight = 0.36
                agencyHeight = 0.14
                isAgencyVisible = true
            }
            else
            {
                agencyHeight = 0.0000000001
            ...
            {
                locationHeight = 0.0000000001
                isLocationVisible = false
            }
            
            if (locArr != nil && agncArr != nil)
            {
                inspectionHeight = 0.55
            }
        }
        else
        {
            ...
            inspectionHeight = 0.0000000001
        }
        let location: NSLayoutConstraint = NSLayoutConstraint.init(item: LocationHigh.firstItem as Any, attribute: LocationHigh.firstAttribute, relatedBy: LocationHigh.relation, toItem: LocationHigh.secondItem, attribute: LocationHigh.secondAttribute, multiplier: locationHeight, constant: 0)
        let agency: NSLayoutConstraint = NSLayoutConstraint.init(item: AgencyHigh.firstItem as Any, attribute: AgencyHigh.firstAttribute, relatedBy: AgencyHigh.relation, toItem: AgencyHigh.secondItem, attribute: AgencyHigh.secondAttribute, multiplier: agencyHeight, constant: 0)
        AgencyHigh.isActive = false
        AgencyHigh = nil
        AgencyHigh = agency
        LocationHigh.isActive = false
        LocationHigh = nil
        LocationHigh = location
        NSLayoutConstraint.activate([AgencyHigh, LocationHigh])
        ...
        let inspection = NSLayoutConstraint.init(item: inspectionHigh.firstItem as Any, attribute: inspectionHigh.firstAttribute, relatedBy: inspectionHigh.relation, toItem: inspectionHigh.secondItem, attribute: inspectionHigh.secondAttribute, multiplier: inspectionHeight, constant: inspectionHigh.constant)
        inspectionHigh.isActive = false
        inspectionHigh = nil
        inspectionHigh = inspection
        NSLayoutConstraint.activate([inspectionHigh, inspectionTop])
    ...
    private func callApiForInspectionOrPayOrForwardQuotation(_Pay p: String, _agency agency: String?, _location location: String?, _mode mode: String?, _payBtn btnString : String)
    {
        ...
        case 2:
            ...
            insurance_type = common.car_insurance
            break
        case 1:
            vehicleType = common.twowheeler
            insurance_type = common.twowheel_insurance
            break
        case 10:
            vehicleType = common.goodCommercial
            insurance_type = common.commercialVehicleInsurance + common.goodsCarry_insurance
            break
        case 9:
            vehicleType = common.passengerCommercial
            insurance_type = common.commercialVehicleInsurance + common.passCarry_insurance
            break
        default:
            vehicleType = common.miscCommercial
            insurance_type = common.commercialVehicleInsurance + common.miscVeh_insurance
            break
        }
        let k = common.A_P_I+common.baseURL+common.forApi+common.motor+vehicleType+common.proposal_req
        ...
        APICallered().POSTMethodForDataToGet(dataToPass: jsonObj, toURL: k) { response in
            ...
            if ((!(resp.object(forKey: "ReferenceId") is NSNull)) && (resp.object(forKey: "Status") as! String).elementsEqual("Success"))
            {
                DispatchQueue.main.async {
                    ...
                    print(jsonObjc)
                    let kr = self.common.buy+self.common.baseURL+self.common.insurance+insurance_type+self.common.proposal_req
                        ...
                        if (response?.object(forKey: "Success") as! Bool)
                        {
                            ...
                            DispatchQueue.main.async {
                                if (btnString.elementsEqual("Forward Quotation"))
                                {
                                    ...
                                    let urlS = self.common.buy+self.common.baseURL+self.common.client+self.common.send_to_client_mob
    ...
    private func loadNextScreeen(_ url : String)
    {
        ...
        let secondViewController = storyboard.instantiateViewController(withIdentifier:"WebViewController") as! WebViewController
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        case 1991:
                ...
                break
        case 1992:
                let c = (payArr[indexPath.row]).count*10
                CellWidth = CGFloat(c)
                CellHeight *= 0.7
                break
        case 1993:
...
//        break
    ...
    {
        let mainURL = common.buy+common.baseURL+common.client
        let k = mainURL+common.conf_val_otp+common.quot_Number+def.string(forKey: "QuotationNumber")!
        let loadingNotification = MBProgressHUD.showAdded(to: view, animated: true)
        loadingNotification.mode = MBProgressHUDMode.indeterminate
        ...
        APICallered().fetchData(k) { response in
            print(response!)
                    DispatchQueue.main.async {
                        var sendOTPTo = mainURL+self.common.gen_otp+self.common.mobileNumb+self.mobile.text!+self.common.productCode+String(self.def.integer(forKey: "ProductCode"))
                        sendOTPTo += self.common.subProductCode+self.def.string(forKey: "SubProductCode")!
                        sendOTPTo += self.common.quotationNumber+self.def.string(forKey: "QuotationNumber")!
                        sendOTPTo += self.common.clientName+self.holderName.text!+self.common.emailId+self.email.text!
                        ...
                        DispatchQueue.main.async {
                                    APICallered().fetchData(k) { response in
                                        DispatchQueue.main.async {
                                            ...
                                            let alertVu = SCLAlertView.init(appearance: self.common.alertWithoutCancel)
    ...
    func hideMyEmailVu(_ hide: Bool) {
        var constraintV: NSLayoutConstraint!
        var constraintT: NSLayoutConstraint!
        if (hide)
        {
            emailTop.constant = 0
            constraintV = NSLayoutConstraint.init(item: emailHigh.firstItem as Any, attribute: emailHigh.firstAttribute, relatedBy: emailHigh.relation, toItem: emailHigh.secondItem, attribute: emailHigh.secondAttribute, multiplier: 0.000001, constant: 0)
            constraintT = NSLayoutConstraint.init(item: paymentHigh.firstItem as Any, attribute: paymentHigh.firstAttribute, relatedBy: paymentHigh.relation, toItem: paymentHigh.secondItem, attribute: paymentHigh.secondAttribute, multiplier: 0.57, constant: paymentHigh.constant)
        ...
        else
        {
            ...
            constraintV = NSLayoutConstraint.init(item: emailHigh.firstItem as Any, attribute: emailHigh.firstAttribute, relatedBy: emailHigh.relation, toItem: emailHigh.secondItem, attribute: emailHigh.secondAttribute, multiplier: 0.1, constant: emailHigh.constant)
            constraintT = NSLayoutConstraint.init(item: paymentHigh.firstItem as Any, attribute: paymentHigh.firstAttribute, relatedBy: paymentHigh.relation, toItem: paymentHigh.secondItem, attribute: paymentHigh.secondAttribute, multiplier: 0.68, constant: paymentHigh.constant)
            payNowBtn.setTitle("Forward Quotation", for: .normal)
        }
        emailHigh.isActive = false
        emailHigh = nil
        emailHigh = constraintV
        constraintV = nil
        paymentHigh.isActive = false
        paymentHigh = nil
        paymentHigh = constraintT
        constraintT = nil
        NSLayoutConstraint.activate([emailHigh, paymentHigh, emailTop])
