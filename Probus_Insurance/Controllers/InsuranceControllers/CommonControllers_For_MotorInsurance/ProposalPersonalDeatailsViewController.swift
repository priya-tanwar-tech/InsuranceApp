...
class ProposalPersonalDeatailsViewController : UIViewController, UIScrollViewDelegate, UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate
{
    ...
    private func showAndHideListView(_ aVu: UIView, _ hide: Bool)
    {
        ...
        {
        case 71://salute
                let tableHigh = NSLayoutConstraint.init(item: saluteTableShow.firstItem as Any, attribute: saluteTableShow.firstAttribute, relatedBy: NSLayoutConstraint.Relation.equal, toItem: saluteTableShow.secondItem, attribute: saluteTableShow.secondAttribute, multiplier: high, constant: saluteTableShow.constant)

                saluteTableShow.isActive = false
                saluteTableShow = nil
                saluteTableShow = tableHigh
                NSLayoutConstraint.activate([saluteTableShow])
                break
        case 73://status
                let tableHigh = NSLayoutConstraint.init(item: statusTableShow.firstItem as Any, attribute: statusTableShow.firstAttribute, relatedBy: NSLayoutConstraint.Relation.equal, toItem: statusTableShow.secondItem, attribute: statusTableShow.secondAttribute, multiplier: high, constant: statusTableShow.constant)

                statusTableShow.isActive = false
                statusTableShow = nil
                statusTableShow = tableHigh
                NSLayoutConstraint.activate([statusTableShow])
                break
        default://occupation
                let tableHigh = NSLayoutConstraint.init(item: occuTableShow.firstItem as Any, attribute: occuTableShow.firstAttribute, relatedBy: NSLayoutConstraint.Relation.equal, toItem: occuTableShow.secondItem, attribute: occuTableShow.secondAttribute, multiplier: high, constant: occuTableShow.constant)

                occuTableShow.isActive = false
                occuTableShow = nil
                occuTableShow = tableHigh
                NSLayoutConstraint.activate([occuTableShow])
                break
        }
        aVu.isHidden = hide
        aVu.layoutIfNeeded()
    ...
    private func loadApiData()
    {
        ...
        {
        case 2:
            vehicle_Type = common.privatecar
            break
        default:
            vehicle_Type = common.twowheeler
            break
        }
        
        let k = def.string(forKey: "ThisCompany")
        
        APICallered().fetchData(common.A_P_I+common.baseURL+common.forApi+common.motor+vehicle_Type+common.salutation+common.company+k!) { response in
            self.SalutationArr = response?.object(forKey: "Response") as? NSArray
        DispatchQueue.main.async {
            self.saluteTableView.reloadData()
            }
        }
        APICallered().fetchData(common.A_P_I+common.baseURL+common.forApi+common.motor+vehicle_Type+common.maritalStatus+common.company+k!) { response in
        ...
        APICallered().fetchData(common.A_P_I+common.baseURL+common.forApi+common.motor+vehicle_Type+common.occupation+common.company+k!) { response in
